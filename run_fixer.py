...
3. Runs the fixer (miner_mineru fix) for each pair
4. Generates corrected markdown + reports in output/fixed/

Note: Requires pre-extracted TOC files in output/ directory.
For extracting TOC from markdown, use: python -m miner_mineru extract <file.md> --output output/toc.json
"""
...
def main():
    ...
    if not toc_files:
        ...
        print("  python -m miner_mineru extract <markdown_file> --output output/<name>.json")
        sys.exit(1)
