...
def process_markdown_file(md_path: str) -> bool:
    ...
    output_json = str(PROJECT_ROOT / "output" / f"{md_name}.json")
    ...
    output_dir = str(PROJECT_ROOT / "output" / "fixed")
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    if not run_command([sys.executable, "-m", "docstruct", "fix", md_path, "--toc", output_json, "--output-dir", output_dir], "Step 2: Fixing heading levels"):
        return False

    report_path = Path(output_dir) / f"{md_name}_report.json"
    if report_path.exists():
        try:
            ...
            print(f"    - Output: output/fixed/{Path(md_path).name}")
